...
## <a name="tls-12-enforcement-available-in-select-regions"></a>La contrainte de TLS 1.2 n’est disponible que dans certaines régions
...
* US Gov Virginie (La prise en charge de TLS 1.0/1.1 n’est pas disponible dans cette région. La contrainte de TLS 1.2 doit être activée, sinon la création d’un hub IoT échoue.)

Pour activer la contrainte de TLS 1.2, suivez les étapes décrites dans [Créer un hub IoT dans Portail Azure](iot-hub-create-through-portal.md), sauf
...
Pour les hubs IoT non configurés pour la contrainte de TLS 1.2, ce protocole fonctionne quand même avec les suites de chiffrement suivantes :
